...
import { calculateNuFastProbs, getProbabilitiesForInitialFlavor } from '../../src/physics/NuFastPort';
...
const createFullParams = (overrides: Partial<typeof defaultOscParams & { L: number, energy: number, initialFlavorIndex: 0 | 1 | 2 }>): OscillationParameters => {
...
describe('NuFastPort Physics Calculations', () => {
    ...
    describe('calculateNuFastProbs', () => {
        ...
        it('should show vacuum oscillations (Pme > 0 for L > 0)', () => {
            const params = createFullParams({ L: 500, energy: 1.0, matterEffect: false, initialFlavorIndex: 1 }); // Muon start
            const result = calculateNuFastProbs(params);
            expect(result[1][0]).toBeGreaterThan(1e-6); // P(mu -> e) should be non-zero
        });

        it('should show matter effects (Vacuum != Matter)', () => {
            const vacParams = createFullParams({ L: 2000, energy: 3.0, matterEffect: false });
            const matParams = createFullParams({ L: 2000, energy: 3.0, matterEffect: true, rho: 2.8 }); // Corrected: Use rho
        ...
        it('should handle antineutrinos (E < 0) differently from neutrinos (E > 0) with CPV and Matter', () => {
            const nuParams = createFullParams({ L: 1300, energy: 2.5, matterEffect: true, rho: 2.8, deltaCP_deg: 270 }); // Corrected: Use rho
            const antiNuParams = createFullParams({ L: 1300, energy: -2.5, matterEffect: true, rho: 2.8, deltaCP_deg: 270 }); // Corrected: Use rho
        ...
        it('should handle NO vs IO (different dm31sq sign)', () => {
            const noParams = createFullParams({ L: 1300, energy: 2.5, matterEffect: true, rho: 2.8, dm31sq_eV2: 2.507e-3 }); // Corrected: Use rho
            const ioParams = createFullParams({ L: 1300, energy: 2.5, matterEffect: true, rho: 2.8, dm31sq_eV2: -2.487e-3 }); // Corrected: Use rho
        ...
        it('should produce plausible value for T2K-like Pme max', () => {
            ...
            console.log(`Calculated T2K-like P(mu->e) at E=0.6GeV, L=295km (vacuum): ${Pme.toFixed(4)}`);
        });
    });

    describe('getProbabilitiesForInitialFlavor', () => {
        it('should return the correct row from the matrix', () => {
            const params = createFullParams({ L: 1000, energy: 1.0, initialFlavorIndex: 1 }); // Muon start
        ...
        it('should return vector summing to 1', () => {
            const params = createFullParams({ L: 1000, energy: 1.0, initialFlavorIndex: 0 }); // Electron start
            // Pass the single params object
            const vector = getProbabilitiesForInitialFlavor(params);
