...
declare global {
    interface Window {
        ...
        physicsEngine?: typeof NuFastPort;
...
export default function createNeutrinoSketch(
    ...
    return (p: P5SketchInstance) => {
        ...
        p.draw = () => {
            ...
            neutrino.updateProbabilities(currentProbs, getDominantFlavorName(currentProbs));
        ...
        function recalculateAndCacheProbabilities(): void {
            ...
            try {
                for (let i = 0; i < numberOfCachePoints; i++) {
                    ...
                    probabilities.push(window.physicsEngine.getProbabilitiesForInitialFlavor(params));
                }
        ...
        function getCurrentProbabilities(): ProbabilityVector {
            ...
            for (let i = 0; i < cachedDistances.length - 1; i++) {
                if (clampedL >= cachedDistances[i] && clampedL <= cachedDistances[i + 1]) {
                    const t = (clampedL - cachedDistances[i]) / (cachedDistances[i + 1] - cachedDistances[i]);
                    return [
                        cachedProbabilities[i][0] + t * (cachedProbabilities[i + 1][0] - cachedProbabilities[i][0]),
                        cachedProbabilities[i][1] + t * (cachedProbabilities[i + 1][1] - cachedProbabilities[i][1]),
                        cachedProbabilities[i][2] + t * (cachedProbabilities[i + 1][2] - cachedProbabilities[i][2])
                    ];
                }
            }
