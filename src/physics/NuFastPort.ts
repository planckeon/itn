...
function probabilityVacuumLBL(
    s12sq: number, s13sq: number, s23sq: number, delta: number,
    Dmsq21: number, Dmsq31: number, L: number, E: number
): ProbabilityMatrix {
    ...
    // --- Guard against E = 0 --- 
    if (absE < 1e-12) { 
        console.warn("Vacuum calc received E near zero. Returning identity matrix.");
    ...
    const Pme_CPC = ((1 - Um3sq - Ue3sq) - Um2sq * Ue1sq - Um1sq * Ue2sq) * sinsqD21_2
...
function probabilityMatterLBL(
    s12sq: number, s13sq: number, s23sq: number, delta: number,
    Dmsq21: number, Dmsq31: number, L: number, E: number,
    rho: number, Ye: number, N_Newton: number
): ProbabilityMatrix {
    ...
    // --- Guard against E = 0 --- 
    if (absE < 1e-12) { 
        console.warn("Matter calc received E near zero. Returning identity matrix.");
        // Return identity matrix for no oscillation
        return [[1, 0, 0], [0, 1, 0], [0, 0, 1]]; 
    }
    ...
    if (Math.abs(Dmsqee_safe) < 1e-30) {
        ...
        const tmp_sqrt = (tmp_sqrt_arg < -1e-12) ? 0 : Math.sqrt(Math.max(0, tmp_sqrt_arg)); // Allow tiny negative due to float errors
    ...
    const Dlambda21 = (Dlambda21_sq_arg < -1e-12) ? 0 : Math.sqrt(Math.max(0, Dlambda21_sq_arg)); // Allow tiny negative
    ...
    const Tmm = Tmm_vac_part * (1 - Um3sq_vac - Um2sq_vac) + Amatter * (See + Smm - A_coeff);
    ...
    const Pme_CPC = (Vt3sq - Um2sq*Ve1sq - Um1sq*Ve2sq) * sinsqD21_2
    ...
    const Pmm = 1 - 2 * (Um1sq*Um2sq*sinsqD21_2 + Um1sq*Um3sq*sinsqD31_2 + Um2sq*Um3sq*sinsqD32_2);
    const Pee = 1 - 2 * (Ve1sq*Ve2sq*sinsqD21_2 + Ve1sq*Ve3sq*sinsqD31_2 + Ve2sq*Ve3sq*sinsqD32_2);
...
export function calculateNuFastProbs(params: Readonly<OscillationParameters>): ProbabilityMatrix {
    ...
    const isValid = 
        !isNaN(params.E) && Math.abs(params.E) > 1e-12 && // Check E is valid and non-zero
        !isNaN(params.L) && params.L >= 0 &&
        !isNaN(params.s12sq) && params.s12sq >= 0 && params.s12sq <= 1 &&
        !isNaN(params.s13sq) && params.s13sq >= 0 && params.s13sq <= 1 &&
        !isNaN(params.s23sq) && params.s23sq >= 0 && params.s23sq <= 1 &&
        !isNaN(params.dm21sq) &&
        !isNaN(params.dm31sq) &&
    ...
    if (!params.matterEffect || Math.abs(params.rho) < 1e-9) {
        ...
        return probabilityVacuumLBL(
            params.s12sq, params.s13sq, params.s23sq, params.deltaCP_rad,
            params.dm21sq, params.dm31sq, params.L, params.E
        );
    } else {
