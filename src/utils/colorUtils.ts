 ...
 */
export function blendNeutrinoColors(probabilities: ProbabilityVector): RGBColor {
  ...
  const normalizedProbs = sum > 0.999 && sum < 1.001 
    ? probabilities 
 ...
 */
export function getDominantFlavor(probabilities: ProbabilityVector): 'electron' | 'muon' | 'tau' {
  const [p_e, p_mu, p_tau] = probabilities;
