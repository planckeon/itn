 ...
 * @returns Formatted number as a string
 */
export function formatScientific(value: number, decimals: number = 2, threshold: number = 1e4): string {
  // Use scientific notation if >= threshold OR > 0 and < a small fixed threshold (e.g., 1e-3)
  const smallThreshold = 1e-3;
  if (Math.abs(value) >= threshold || (Math.abs(value) > 0 && Math.abs(value) < smallThreshold)) {
