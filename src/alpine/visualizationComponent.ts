import type { OscillationParameters, ProbabilityVector, TooltipState, PlotParameters } from '../physics/types';
...
export function neutrinoVisualizationComponent() {
  ...
  const getPhysicsParams = (lValue: number): OscillationParameters => {
      ...
      return {
          ...
          matterEffect: simParams.matterEffect, initialFlavorIndex: simParams.initialFlavorIndex, N_Newton: 0
      };
  };
  ...
  return {
    ...
    toggleTooltip(name: string) {
      ...
      for (const key in tooltipState.visible) { tooltipState.visible[key] = false; }
      ...
      if (tooltipState.visible[name]) {
          ...
          this.$nextTick(() => {
              // @ts-ignore: Alpine magic property
              const tooltipElement = this.$el.querySelector(`.tooltip-content[data-tooltip-id="${name}"]`);
              if (tooltipElement && window.katex) {
                  tooltipElement.querySelectorAll('[x-katex]').forEach((katexEl: Element) => {
    ...
    updateCurrentProbsAndFlavor(lValue: number) {
         ...
         const probs = window.physicsEngine.getProbabilitiesForInitialFlavor(params);
