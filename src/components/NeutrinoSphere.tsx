		...
		const render = () => {
			...
			if (colorTransitionRef.current < 1) {
				colorTransitionRef.current = Math.min(
					1,
					colorTransitionRef.current + 0.05,
				);
			}

			// Check if target color changed significantly
			const colorDiff =
				Math.abs(targetColor.r - (prevColorRef.current?.r || 0)) +
			...
			const displayColor =
				prevColorRef.current && colorTransitionRef.current < 1
					? lerpColor(
			...
			for (let i = 0; i < numParticles; i++) {
				...
				const theta =
					...
					timeRef.current * 0.4 * matterSpeedMult;
				const phi = Math.acos(1 - 2 * ((i * 0.618033988749895) % 1));

				// Animated wobble
				const wobble =
					0.12 * matterWobbleMult * Math.sin(timeRef.current * 2.5 + i * 0.7);
				const particleRadius =
					radius *
					(1.08 + wobble + 0.1 * Math.sin(theta * 2 + timeRef.current * 1.5));
			...
			sphereGradient.addColorStop(
				0,
				`rgb(${highlight.r}, ${highlight.g}, ${highlight.b})`,
			);
			sphereGradient.addColorStop(0.4, `rgb(${r}, ${g}, ${b})`);
			sphereGradient.addColorStop(
				1,
				`rgb(${shadow.r}, ${shadow.g}, ${shadow.b})`,
			);
			...
			const specularGradient = ctx.createRadialGradient(
				centerX + lightOffsetX * 0.7,
				centerY + lightOffsetY * 0.7,
				0,
				centerX + lightOffsetX * 0.4,
				centerY + lightOffsetY * 0.4,
				radius * 0.45,
			);
			...
			if (isNearResonance) {
				...
				ringGlow.addColorStop(
					0.4,
					`rgba(255, 200, 50, ${resonanceStrength * 0.25})`,
				);
